...
const Dashboard = () => {
    ...
    const getExerciseDetails = (workout) => {
        ...
        return workout.workout_exercises.map(we => ({
            ...
            sets: we.sets || (we.exercise.exercise_type === 'strength' ? [{reps: 1, weight: 0}] : [{
                distance: 0,
                time: 0
            }])
        }));
                        ...
                        reversedWorkouts.map((workout) => (
                            <div key={workout.id}
                                ...
                                <div
                                    ...
                                    <h3 className="text-left text-white text-lg md:text-xl font-bold mb-2 md:mb-0 w-10/12 truncate">
                                ...
                                <div className="text-gray-400 text-xs md:text-sm text-left">
                                    <div className="flex items-center space-x-2">
                                        <span>{format(new Date(workout.date_started), 'p')}</span>
                                        <span className="text-gray-500">-</span>
                                        <span>{format(new Date(workout.date_started), 'PPP')}</span>
                                    </div>
                                </div>
                                            ...
                                            <div key={exerciseIndex}>
                                                ...
                                                {exercise.sets.length > 0 ? (
                                                    exercise.sets.map((set, setIndex) => (
                                                        <div key={setIndex}
                                                             className="ml-4 text-gray-400 text-xs md:text-sm">
                                                            {exercise.exercise_type === 'strength' ? (
                                                                <p>Set {setIndex + 1}: Reps: {set.reps},
                                                                    Weight: {set.weight} kg</p>
                                                            ) : (
                                                                <p>Set {setIndex + 1}: Distance: {set.distance} km,
                                                                    Time: {set.time} min</p>
                                                            )}
                                                        </div>
                                                    ))
                                                ) : (
                                                    <p className="text-gray-400 text-xs md:text-sm ml-4">No sets
                                                        available.</p>
                                                )}
                                            </div>
                                        ))
